...
module.exports = {
    ...
    async getAllByUsuarioId(usuario_id) {
        let alugueis = await db(TABLE).where({ usuario_id }).whereNotNull('paypal_order_id').orWhereNotNull('subscription_id').orderBy('data_criacao');
        for (let aluguel of alugueis) {
            aluguel = await getMoreDetails(aluguel);
        }
        return alugueis;
    ...
    async getAllAlocacoesByUsuarioId(anunciante_id) {
        let alugueis = await db.select(`${TABLE}.*`).from(TABLE)
            ...
            .where(`alugavel.anunciante_id`, anunciante_id).whereNotNull('paypal_order_id').orWhereNotNull('subscription_id').orderBy(`dias_reservados.data_entrada`, 'desc');
